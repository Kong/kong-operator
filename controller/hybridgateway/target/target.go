...
func createTargetsFromValidBackendRefs(ctx context.Context, logger logr.Logger, cl client.Client, httpRoute *gwtypes.HTTPRoute, pRef *gwtypes.ParentReference, upstreamName string,
	...
	for _, vbRef := range validBackendRefs {
		...
		for _, endpoint := range vbRef.readyEndpoints {
			...
			target.Annotations[consts.GatewayOperatorHybridRouteAnnotation] = existingTarget.Annotations[consts.GatewayOperatorHybridRouteAnnotation]
			annotationManager := annotations.NewAnnotationManager(logger)
			_, err = annotationManager.AppendHTTPRouteToAnnotation(&target, httpRoute)
			if err != nil {
				log.Error(logger, err, "Failed to update HTTPRoute annotation on KongTarget")
				return nil, fmt.Errorf("failed to add HTTPRoute annotation on KongTarget %s: %w", targetName, err)
			}

			// TODO: we should check that the existingTarget.Spec matches what we expect and error out if not
