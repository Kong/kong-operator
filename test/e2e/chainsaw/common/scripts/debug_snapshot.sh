...
#
# NOTE: Sections are ordered by debugging value (events, operator logs first),
# so that a snapshot aborted early — e.g. by a CI runner killing the job after
# a timeout — still contains the most important data.
...
#   SKIP_RESOURCE_COUNTS: If set to 1, skip the (slow) per-namespace resource
#     counting in the summary section (optional).
#   KUBECTL_TIMEOUT: Per-command timeout in seconds for each kubectl call
#     (default: 15).
...
SKIP_RESOURCE_COUNTS="${SKIP_RESOURCE_COUNTS:-0}"

# Bound each kubectl call so one hung call cannot eat the whole dump
# (CI allows only ~5 minutes of steps after a job timeout/cancellation).
KUBECTL_TIMEOUT="${KUBECTL_TIMEOUT:-15}"
kc() {
  if command -v timeout >/dev/null 2>&1; then
    timeout "${KUBECTL_TIMEOUT}" kubectl "$@"
  else
    kubectl "$@"  # no coreutils `timeout` (e.g. macOS): no per-call bound
  fi
}
...
# Function to safely run kc commands and capture output
safe_kubectl() {
  local output_file=$1
  shift
...
# 1. Capture events in test namespaces
...
# 2. Capture events in operator namespace
...
OPERATOR_PODS=$(kc get pods -n "${OPERATOR_NAMESPACE}" -l app.kubernetes.io/instance=kong-operator -o jsonpath='{.items[*].metadata.name}' 2>/dev/null || echo "")
...
if [ -n "${OPERATOR_PODS}" ]; then
  for pod in ${OPERATOR_PODS}; do
    ...
    if kc logs "${pod}" -n "${OPERATOR_NAMESPACE}" --all-containers --timestamps >> "${OPERATOR_LOGS_FILE}" 2>&1; then
      echo "" >> "${OPERATOR_LOGS_FILE}"
    else
    ...
    if kc logs "${pod}" -n "${OPERATOR_NAMESPACE}" --all-containers --timestamps --previous >> "${OPERATOR_LOGS_FILE}" 2>&1; then
      echo "" >> "${OPERATOR_LOGS_FILE}"
    else
...
# 5. Capture all resources in test namespaces (YAML format)
...
for ns in ${ALL_NAMESPACES}; do
  ...
  capture_resource_group "${RESOURCES_FILE}" "${ns}" "Konnect Resources" \
    "konnectgatewaycontrolplanes.konnect.konghq.com,konnectextensions.konnect.konghq.com,konnectconfigstores.konnect.konghq.com"
  ...
  if kc get konnectapiauthconfigurations.konnect.konghq.com -n "${ns}" -o yaml 2>/dev/null | \
...
for ns in ${ALL_NAMESPACES}; do
  ...
  safe_kubectl "${DESCRIBED_FILE}" describe gateways,gatewayclasses,tlsroutes,httproutes,udproutes,referencegrants -n "${ns}"
  ...
  safe_kubectl "${DESCRIBED_FILE}" describe kongcertificates,kongsnis,kongroutes,kongservices,kongupstreams,kongtargets,kongvaults,kongreferencegrants,kongplugins -n "${ns}"
  ...
  safe_kubectl "${DESCRIBED_FILE}" describe konnectgatewaycontrolplanes,konnectextensions,konnectconfigstores -n "${ns}"
  ...
  if kc describe konnectapiauthconfigurations -n "${ns}" 2>/dev/null | \
...
for ns in ${ALL_NAMESPACES}; do
  ...
  PODS=$(kc get pods -n "${ns}" -o jsonpath='{.items[*].metadata.name}' 2>/dev/null || echo "")
  if [ -z "${PODS}" ]; then
  ...
  for pod in ${PODS}; do
    ...
    if kc logs "${pod}" -n "${ns}" --all-containers --timestamps >> "${WORKLOAD_LOGS_FILE}" 2>&1; then
      echo "" >> "${WORKLOAD_LOGS_FILE}"
    else
    ...
    if kc logs "${pod}" -n "${ns}" --all-containers --timestamps --previous >> "${WORKLOAD_LOGS_FILE}" 2>&1; then
      echo "" >> "${WORKLOAD_LOGS_FILE}"
    else
...
# Count resources by namespace (skippable: one kc call per resource type
# per namespace — minutes on busy clusters).
if [ "${SKIP_RESOURCE_COUNTS}" != "1" ]; then
  for ns in ${ALL_NAMESPACES}; do
...
echo "Operator pod count: $(kc get pods -n "${OPERATOR_NAMESPACE}" -l app.kubernetes.io/instance=kong-operator --no-headers 2>/dev/null | wc -l | tr -d ' ')" >> "${SUMMARY_FILE}"
