...
// +kubebuilder:validation:XValidation:rule="(!self.status.conditions.exists(c, c.type == 'Programmed' && c.status == 'True')) ? true : ((!has(oldSelf.spec.id) && !has(self.spec.id)) || (has(oldSelf.spec.id) && has(self.spec.id) && oldSelf.spec.id == self.spec.id))", message="spec.id is immutable when an entity is already Programmed"
...
type KongCACertificateSpec struct {
	...
	// ID is the unique identifier for the KongCACertificate.
	// When it is specified, the ID is used as the identifier for the KongCACertificate in Konnect.
	// If not specified, a unique ID will be generated by Konnect on creation.
	// +optional
