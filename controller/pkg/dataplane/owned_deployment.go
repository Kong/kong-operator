...
// rolloutOntoCertificateComplete reports whether the DataPlane Deployment
// finished rolling out the given certificate checksum: every replica runs the
// current Pod template and the template carries the current checksum
...
func (r *Reconciler[T, CP, Cert]) rolloutOntoCertificateComplete(
	...
	if err := r.Get(ctx, client.ObjectKey{Namespace: dp.GetNamespace(), Name: dp.GetName()}, deployment); err != nil {
...
func GenerateBaseDeployment[T Object, CP ControlPlaneObject, Cert CertificateObject](
	...
	// No cert Secret was provisioned: omit the volume too, a Pod can't mount
	// a volume that doesn't exist.
	if certSecretName != "" {
		volumes = append(
			volumes,
			corev1.Volume{
...
// certChecksumAnnotation returns the pod-template annotation carrying the
// certificate content checksum, or nil when checksum tracking is not
// configured. Changing the checksum changes the Pod template, which rolls
// the Deployment when the certificate Secret is rotated in place.
func certChecksumAnnotation(checksum, key string) map[string]string {
	if checksum == "" || key == "" {
		return nil
	}
	return map[string]string{key: checksum}
}
