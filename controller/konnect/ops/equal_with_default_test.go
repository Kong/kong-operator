...
func TestEqualWithDefault_Int(t *testing.T) {
	...
	require.True(t, equalWithDefault[int](a, b, def))

	// nil vs value equal to default
	v := def
	b = &v
	assert.True(t, equalWithDefault[int](nil, b, def))

	// nil vs value not equal to default
	w := def + 1
	b = &w
	assert.False(t, equalWithDefault[int](nil, b, def))

	// zero value vs nil -> default vs default
	z := 0
	assert.True(t, equalWithDefault[int](&z, nil, def))

	// equal non-zero values
	aVal := 123
	bVal := 123
	assert.True(t, equalWithDefault[int](&aVal, &bVal, def))

	// different non-zero values
	bVal = 321
	assert.False(t, equalWithDefault[int](&aVal, &bVal, def))
}
...
func TestEqualWithDefault_String(t *testing.T) {
	...
	// both nil
	assert.True(t, equalWithDefault[string](nil, nil, def))

	// empty string vs nil -> default("") vs default("")
	empty := ""
	assert.True(t, equalWithDefault[string](&empty, nil, def))

	// equal non-empty strings
	s1 := "foo"
	s2 := "foo"
	assert.True(t, equalWithDefault[string](&s1, &s2, def))

	// different non-empty strings
	s2 = "bar"
	assert.False(t, equalWithDefault[string](&s1, &s2, def))

	// non-empty vs default fallback
	def2 := "/"
	slash := "/"
	assert.True(t, equalWithDefault[string](&empty, &slash, def2))
}
