...
func TestGenerateCommonTypes(t *testing.T) {
	...
	t.Run("with GetKonnectStatus override aliases custom status type", func(t *testing.T) {
		g := NewGenerator(Config{
			APIVersion: "v1alpha1",
			FuncsConfig: map[string]*config.EntityFuncsConfig{
				"Portal": {
					Funcs: map[string]*config.FuncConfig{
						"GetKonnectStatus": {
							ReturnType: &config.TypeRefConfig{
								Package: "github.com/example/custom/status",
								Alias:   "customstatus",
								Type:    "EntityStatus",
							},
						},
		...
		content, err := g.generateCommonTypes()
		require.NoError(t, err)
		assert.Contains(t, content, `customstatus "github.com/example/custom/status"`)
		assert.Contains(t, content, "type KonnectEntityStatus = customstatus.EntityStatus")
	})
}
...
func TestGenerateCRDFuncs_GeneratesKonnectFuncs(t *testing.T) {
	...
	t.Run("custom Konnect status return type override", func(t *testing.T) {
		g := NewGenerator(Config{
			APIGroup:   "x-konnect.konghq.com",
			APIVersion: "v1alpha1",
			FuncsConfig: map[string]*config.EntityFuncsConfig{
		...
		require.NoError(t, err)
		assert.Contains(t, content, `customstatus "github.com/example/custom/status"`)
		assert.Contains(t, content, "func (obj *Portal) GetKonnectStatus() *customstatus.EntityStatus {")
	})
	...
	t.Run("root reconciler entities keep custom Konnect status imports alongside auth ref import", func(t *testing.T) {
		g := NewGenerator(Config{
			APIGroup:   "x-konnect.konghq.com",
			APIVersion: "v1alpha1",
			ReconcilerConfig: map[string]*config.ReconcilerConfig{
...
func TestGenerateCommonTypes_ConflictingGetKonnectStatusOverrides(t *testing.T) {
	g := NewGenerator(Config{
		APIVersion: "v1alpha1",
		FuncsConfig: map[string]*config.EntityFuncsConfig{
			"Portal": {
			...
			"PortalTeam": {
				Funcs: map[string]*config.FuncConfig{
					"GetKonnectStatus": {
						ReturnType: &config.TypeRefConfig{
							Package: "github.com/example/other/status",
							Alias:   "otherstatus",
							Type:    "EntityStatus",
						},
					},
	...
	_, err := g.generateCommonTypes()
	require.ErrorContains(t, err, "GetKonnectStatus returnType override")
}
