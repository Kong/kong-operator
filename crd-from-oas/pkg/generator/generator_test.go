...
func TestGenerateCommonTypes(t *testing.T) {
	...
	t.Run("with secret ref value validations adds SensitiveDataSource markers", func(t *testing.T) {
		g := NewGenerator(Config{
			APIVersion: "v1alpha1",
		...
		content, err := g.generateCommonTypes(map[string]*config.FieldConfig{
			sensitiveDataSourceTypeName: {
				Fields: map[string]*config.FieldConfig{
					"value": {Validations: []string{"+kubebuilder:validation:MaxLength=4096"}},
				},
			},
		})
		require.NoError(t, err)
		assert.Contains(t, content, "type SensitiveDataSource struct")
		assert.Contains(t, content, "// +kubebuilder:validation:MaxLength=4096")
		assert.Contains(t, content, "Value *string `json:\"value,omitempty\"`")
	})
...
func TestBuildSchemaCursors_SecretReferenceValueUsesSensitiveDataSource(t *testing.T) {
	parsed := &parser.ParsedSpec{
		RequestBodies: map[string]*parser.Schema{
			"CreateListenerPolicy": {
				Name: "CreateListenerPolicy",
				Properties: []*parser.Property{{
					Name:    "certificate",
					Type:    "string",
					RefName: "GatewaySecretReferenceOrLiteral",
				}},
			},
			"CreateBackendCluster": {
				Name: "CreateBackendCluster",
				Properties: []*parser.Property{{
					Name:    "certificate",
					Type:    "string",
					RefName: "GatewaySecretReferenceOrLiteral",
	...
	fieldCfg := &config.Config{
		Entities: map[string]*config.EntityConfig{
			"ListenerPolicy": {Fields: map[string]*config.FieldConfig{
				"spec": {Fields: map[string]*config.FieldConfig{
					"apiSpec": {Fields: map[string]*config.FieldConfig{
						"certificate": {Validations: []string{"+kubebuilder:validation:MaxLength=4096"}},
					}},
			...
			"BackendCluster": {Fields: map[string]*config.FieldConfig{
				"spec": {Fields: map[string]*config.FieldConfig{
					"apiSpec": {Fields: map[string]*config.FieldConfig{
						"certificate": {Fields: map[string]*config.FieldConfig{
							"value": {Validations: []string{"+kubebuilder:validation:MaxLength=4096"}},
						}},
					}},
				}},
	...
	cursors, err := gen.buildSchemaCursors(parsed)
	require.NoError(t, err)
	assert.NotContains(t, cursors, "GatewaySecretReferenceOrLiteral")

	sensitiveCursor := cursors[sensitiveDataSourceTypeName]
	require.NotNil(t, sensitiveCursor)
	valueCursor := sensitiveCursor.Fields["value"]
	require.NotNil(t, valueCursor)
	assert.Equal(t, []string{"+kubebuilder:validation:MaxLength=4096"}, valueCursor.Validations)
}
