...
func newTestReconciler(cl client.Client, recorder *events.FakeRecorder) *Reconciler {
	return &Reconciler{
		Client:                   cl,
...
func TestReconciler_Reconcile(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "certificate secret just created: first reconcile requeues, CertificateProvisioned=True",
			objects: []client.Object{
				newReconcileEGDP(),
				newProgrammedKEG(),
				caSecret(),
		...
		},
		{
			...
			// 1st reconcile: cert created → Requeue: true.
