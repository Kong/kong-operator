...
func New(m metadata.Info) *CLI {
	...
	flagSet.BoolVar(&cfg.EnforceConfig, "enforce-config", true,
		"Enforce the configuration on generated in cluster resources. If set to false, the operator will only enforce the configuration when the owner resource spec changes.")
