...
func init() {
	// Ensure scheme is properly initialized for tests
	_ = scheme.Get()
}
...
func TestKongRouteBuilder_WithLabels(t *testing.T) {
	...
	// We don't test the exact content since that's tested in metadata package
...
func TestKongRouteBuilder_Build(t *testing.T) {
	...
	t.Run("build with errors", func(t *testing.T) {
		builder := NewKongRoute().WithOwner(nil) // This will add an error
