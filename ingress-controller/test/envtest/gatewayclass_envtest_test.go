...
func TestGatewayWithGatewayClassReconciliationUnmanaged(t *testing.T) {
	t.Parallel()
...
func testGatewayWithGatewayClassReconciliation(
	...
	for _, tc := range testcases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			require.NoError(t, cl.Create(ctx, &tc.Gateway))
			t.Cleanup(func() { _ = cl.Delete(t.Context(), &tc.Gateway) })
