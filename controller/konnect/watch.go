...
// and we need to reconcile the objects that are affected by it.
...
// secretRefsForSensitiveData extracts the active Secret references from obj's
...
func secretRefsForSensitiveData(obj any) ([]commonv1alpha1.NamespacedRef, bool) {
	...
	case interface {
		...
		for i, r := range refs {
			out[i] = commonv1alpha1.NamespacedRef{Name: r.Name, Namespace: r.Namespace}
		}
		return out, true
	case interface {
...
// enqueueObjectsForSecretRef returns a function that enqueues reconcile.Requests
// for all objects of type T whose generated GetSensitiveDataSecretRefs()
// references the changed Secret, so that editing the Secret's data promptly
// retriggers reconciliation of every entity depending on it. This covers both
// the shared (string-valued) SensitiveDataSource and dedicated per-field
// DataSource types generated by crd-from-oas, since GetSensitiveDataSecretRefs
// is emitted identically for either.
func enqueueObjectsForSecretRef[
	TList interface {
		GetItems() []T
	},
	TListPtr interface {
	...
	return func(ctx context.Context, obj client.Object) []reconcile.Request {
		...
		if err := cl.List(ctx, lPtr); err != nil {
			return nil
		}
