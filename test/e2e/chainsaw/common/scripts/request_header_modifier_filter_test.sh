...
# Function to validate headers in response.
validate_headers() {
  local RESPONSE="$1"
  local HEADER_ERRORS=""
...
for ATTEMPT in $(seq 1 $MAX_RETRIES); do
  if OUTPUT=$(eval $CURL_CMD 2>&1); then
    ...
    # Validate headers.
    HEADER_ERRORS=$(validate_headers "$RESPONSE")
...
{
  "success": false,
  "error": "RequestHeaderModifier validation failed after $MAX_RETRIES attempts",
