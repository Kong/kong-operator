	...
	if shouldCreateKonnectEntity(ent) {
		...
		if err != nil {
			...
			case okReferenceErr:
				...
				if _, hasHTTPRoute := annotations[consts.GatewayOperatorHybridRoutesHTTPRouteAnnotation]; hasHTTPRoute {
					return ctrl.Result{RequeueAfter: referenceErr.RetryAfter}, nil
				}
				if _, hasTLSRoute := annotations[consts.GatewayOperatorHybridRoutesTLSRouteAnnotation]; hasTLSRoute {
					return ctrl.Result{RequeueAfter: referenceErr.RetryAfter}, nil
				}
				return ctrl.Result{}, nil
			}
...
// HybridGateway controller on behalf of an HTTPRoute or TLSRoute, as indicated
// by the presence of the corresponding ownership annotations.
func isHybridGatewayManaged(obj client.Object) bool {
	annotations := obj.GetAnnotations()
	if _, ok := annotations[consts.GatewayOperatorHybridRoutesHTTPRouteAnnotation]; ok {
		return true
	}
	_, ok := annotations[consts.GatewayOperatorHybridRoutesTLSRouteAnnotation]
	return ok
}
