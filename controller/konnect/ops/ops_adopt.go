...
func AdoptMatch[
	...
	adoptable, ok := any(ent).(interface {
...
func adoptKonnectCloudGatewayNetworkMatch(
	...
	if diffs := compareNetworkSpec(n.Spec, resp.Network); len(diffs) > 0 {
		return fmt.Errorf("spec does not match Konnect entity: %s", strings.Join(diffs, "; "))
	}
...
func adoptKonnectDataPlaneGroupConfigurationMatch(
	...
	if errWrap := wrapErrIfKonnectOpFailed(err, GetOp, cfg); errWrap != nil {
		return errWrap
	}
...
func adoptKonnectTransitGatewayMatch(
	...
	if errWrap := wrapErrIfKonnectOpFailed(err, GetOp, tg); errWrap != nil {
		return errWrap
	}
