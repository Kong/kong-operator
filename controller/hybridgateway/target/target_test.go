...
func TestFindBackendRefPortInService(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			backendRef: createTestBackendRef(new(int32(80))),
		...
		},
		{
			...
			backendRef: createTestBackendRef(new(int32(443))),
			service: createSvc("test-service", "default", []corev1.ServicePort{
				{
					Name:     "http",
					Port:     80,
		...
		},
		{
			...
			backendRef: createTestBackendRef(new(int32(53))),
...
func TestFiltervalidBackendRefs(t *testing.T) {
	...
	tests := []struct {
		...
		validateResults        func(t *testing.T, results []validBackendRef[gwtypes.HTTPBackendRef])
	}{
		{
			name:      "Valid backend ref with regular service",
			httpRoute: createTestHTTPRoute("test-route", "default"),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("test-service", nil, new(int32(80))),
			},
			fqdn: false,
			existingServices: []corev1.Service{
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "default"),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("external-service", nil, new(int32(443))),
			},
			fqdn: false,
			existingServices: []corev1.Service{
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "default"),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("headless-service", nil, new(int32(80))),
			},
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "default"),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("missing-service", nil, new(int32(80))),
			},
			fqdn:               false,
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "default"),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("test-service", nil, new(int32(9999))), // Port not in service.
			},
			fqdn: false,
			existingServices: []corev1.Service{
				*createTestService("test-service", "default", corev1.ServiceTypeClusterIP, "10.0.0.1", "", []corev1.ServicePort{
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "default"),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("no-endpoints-service", nil, new(int32(80))),
			},
			fqdn: false,
			existingServices: []corev1.Service{
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "default"),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("empty-external-service", nil, new(int32(443))),
			},
			fqdn: false,
			existingServices: []corev1.Service{
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "default"),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("valid-service", nil, new(int32(80))),
				createTestHTTPBackendRef("missing-service", nil, new(int32(80))),
				createTestHTTPBackendRef("valid-service", nil, new(int32(443))), // Invalid port.
			},
			fqdn: false,
			existingServices: []corev1.Service{
		...
		},
		{
			...
			backendRefs: []gwtypes.HTTPBackendRef{
				func() gwtypes.HTTPBackendRef {
					...
					ref := createTestHTTPBackendRef("test-configmap", nil, new(int32(80)))
	...
	t.Run("ReferenceGrant enabled scenarios", func(t *testing.T) {
		...
		t.Run("No ReferenceGrant exists", func(t *testing.T) {
			...
			httpRoute := createTestHTTPRoute("test-route", "default")
			backendRefs := []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("cross-ns-service", new("other-namespace"), new(int32(80))),
			}
	...
	t.Run("Service with EndpointSlices NotFound should be skipped", func(t *testing.T) {
		...
		httpRoute := createTestHTTPRoute("test-route", "default")
		backendRefs := []gwtypes.HTTPBackendRef{
			createTestHTTPBackendRef("no-endpointslices-service", nil, new(int32(80))),
		}
...
func TestRecalculateWeightsAcrossBackendRefs(t *testing.T) {
	...
	tests := []struct {
		name           string
		input          []validBackendRef[gwtypes.HTTPBackendRef]
		validateResult func(t *testing.T, result []validBackendRef[gwtypes.HTTPBackendRef])
	}{
		{
		...
		},
		{
			...
			input: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestValidBackendRef("service1", "default", new(int32(100)), []string{"10.0.1.1", "10.0.1.2"}),
			},
			validateResult: func(t *testing.T, result []validBackendRef[gwtypes.HTTPBackendRef]) {
		...
		},
		{
			...
			input: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestValidBackendRef("service1", "default", new(int32(50)), []string{"10.0.1.1"}),
				createTestValidBackendRef("service2", "default", new(int32(50)), []string{"10.0.2.1"}),
			},
			validateResult: func(t *testing.T, result []validBackendRef[gwtypes.HTTPBackendRef]) {
		...
		},
		{
			...
			input: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestValidBackendRef("service1", "default", new(int32(80)), []string{"10.0.1.1"}),
				createTestValidBackendRef("service2", "default", new(int32(20)), []string{"10.0.2.1"}),
			},
			validateResult: func(t *testing.T, result []validBackendRef[gwtypes.HTTPBackendRef]) {
				require.Len(t, result, 2)
		...
		},
		{
			...
			input: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestValidBackendRef("service1", "default", new(int32(50)), []string{"10.0.1.1", "10.0.1.2"}), // 2 endpoints.
				createTestValidBackendRef("service2", "default", new(int32(50)), []string{"10.0.2.1"}),             // 1 endpoint.
			},
			validateResult: func(t *testing.T, result []validBackendRef[gwtypes.HTTPBackendRef]) {
				require.Len(t, result, 2)
		...
		},
		{
			...
			input: []validBackendRef[gwtypes.HTTPBackendRef]{
				...
				createTestValidBackendRef("service2", "default", new(int32(3)), []string{"10.0.2.1"}), // Weight 3.
			},
			validateResult: func(t *testing.T, result []validBackendRef[gwtypes.HTTPBackendRef]) {
				require.Len(t, result, 2)
		...
		},
		{
			...
			input: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestValidBackendRef("service-with-endpoints", "default", new(int32(50)), []string{"10.0.1.1"}),
				createTestValidBackendRef("service-no-endpoints", "default", new(int32(50)), []string{}), // No endpoints.
			},
			validateResult: func(t *testing.T, result []validBackendRef[gwtypes.HTTPBackendRef]) {
				require.Len(t, result, 2)
		...
		},
		{
			...
			input: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestValidBackendRef("service-normal", "default", new(int32(50)), []string{"10.0.1.1"}),
				createTestValidBackendRef("service-zero-weight", "default", new(int32(0)), []string{"10.0.2.1"}), // Zero weight.
			},
			validateResult: func(t *testing.T, result []validBackendRef[gwtypes.HTTPBackendRef]) {
				require.Len(t, result, 2)
		...
		},
		{
			...
			input: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestValidBackendRef("web", "frontend", new(int32(60)), []string{"10.0.1.1", "10.0.1.2", "10.0.1.3"}), // 3 endpoints.
				createTestValidBackendRef("api", "backend", new(int32(30)), []string{"10.0.2.1", "10.0.2.2"}),              // 2 endpoints.
				createTestValidBackendRef("cache", "backend", new(int32(10)), []string{"10.0.3.1"}),                        // 1 endpoint.
			},
			validateResult: func(t *testing.T, result []validBackendRef[gwtypes.HTTPBackendRef]) {
...
func TestCreateTargetsFromvalidBackendRefs(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			upstreamName: "test-upstream",
			validBackendRefs: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestvalidBackendRef("service1", "test-namespace", new(int32(50)), []string{}), // No endpoints.
			},
			expectedTargets: 0,
			expectedError:   false,
		},
		...
		},
		{
			...
			upstreamName: "test-upstream",
			validBackendRefs: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestvalidBackendRef("service1", "test-namespace", new(int32(50)), []string{"10.0.0.1", "10.0.0.2", "10.0.0.3"}),
			},
			expectedTargets: 3,
			expectedError:   false,
			validateResult: func(t *testing.T, targets []configurationv1alpha1.KongTarget) {
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "test-namespace", []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("service1", "test-namespace", nil, new(int32(80))),
				createTestHTTPBackendRef("service2", "test-namespace", nil, new(int32(90))),
			}),
			pRef:         &gwtypes.ParentReference{Name: "test-gateway"},
			upstreamName: "test-upstream",
			validBackendRefs: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestvalidBackendRef("service1", "test-namespace", new(int32(30)), []string{"10.0.1.1", "10.0.1.2"}),
				createTestvalidBackendRef("service2", "test-namespace", new(int32(70)), []string{"10.0.2.1"}),
			},
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "test-namespace", []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("service1", "test-namespace", nil, new(int32(80))),
				createTestHTTPBackendRef("service2", "test-namespace", nil, new(int32(90))),
				createTestHTTPBackendRef("service3", "test-namespace", nil, new(int32(85))),
			}),
			pRef:         &gwtypes.ParentReference{Name: "test-gateway"},
			upstreamName: "test-upstream",
			validBackendRefs: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestvalidBackendRef("service1", "test-namespace", new(int32(0)), []string{}),                        // No endpoints, should be skipped.
				createTestvalidBackendRef("service2", "test-namespace", new(int32(60)), []string{"10.0.2.1"}),             // 1 endpoint.
				createTestvalidBackendRef("service3", "test-namespace", new(int32(40)), []string{"10.0.3.1", "10.0.3.2"}), // 2 endpoints.
			},
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "test-namespace", []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("service1", "test-namespace", nil, new(int32(80))),
				createTestHTTPBackendRef("service2", "test-namespace", nil, new(int32(80))),
			}),
			pRef:         &gwtypes.ParentReference{Name: "test-gateway"},
			upstreamName: "test-upstream",
			validBackendRefs: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestvalidBackendRef("service1", "test-namespace", new(int32(30)), []string{"10.0.0.1"}),
				createTestvalidBackendRef("service2", "test-namespace", new(int32(70)), []string{"10.0.0.1"}), // Same IP.
			},
			expectedTargets: 1,
			expectedError:   false,
			validateResult: func(t *testing.T, targets []configurationv1alpha1.KongTarget) {
		...
		},
		{
			...
			upstreamName: "test-upstream",
			validBackendRefs: []validBackendRef[gwtypes.HTTPBackendRef]{
				createTestvalidBackendRef("service1", "test-namespace", new(int32(40)), []string{"10.0.0.1", "10.0.0.2"}),
				createTestvalidBackendRef("service2", "test-namespace", new(int32(60)), []string{"10.0.0.2", "10.0.0.3"}), // 10.0.0.2 overlaps.
			},
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "test-namespace", []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("service1", "test-namespace", nil, new(int32(8080))),
			}),
			pRef:         &gwtypes.ParentReference{Name: "test-gateway"},
			upstreamName: "test-upstream",
			validBackendRefs: []validBackendRef[gwtypes.HTTPBackendRef]{
...
func TestTargetsForBackendRefs(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "test-namespace", []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("test-service", "other-namespace", nil, new(int32(80))),
			}),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("test-service", "other-namespace", nil, new(int32(80))),
			},
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "test-namespace", []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("service1", "", new(int32(70)), new(int32(80))),
				createTestHTTPBackendRef("service2", "", new(int32(30)), new(int32(80))),
			}),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("service1", "", new(int32(70)), new(int32(80))),
				createTestHTTPBackendRef("service2", "", new(int32(30)), new(int32(80))),
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "test-namespace", []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("service-a", "", new(int32(50)), new(int32(80))),
				createTestHTTPBackendRef("service-b", "", new(int32(50)), new(int32(80))),
			}),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("service-a", "", new(int32(50)), new(int32(80))),
				createTestHTTPBackendRef("service-b", "", new(int32(50)), new(int32(80))),
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "frontend-ns", []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("backend-service", "backend-ns", nil, new(int32(80))),
			}),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("backend-service", "backend-ns", nil, new(int32(80))),
			},
		...
		},
		{
			...
			httpRoute: createTestHTTPRoute("test-route", "frontend-ns", []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("app-backend-v2", "backend-ns", nil, new(int32(80))),
				createTestHTTPBackendRef("app-backend-v1", "backend-ns", nil, new(int32(80))),
			}),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("app-backend-v2", "backend-ns", nil, new(int32(80))),
				createTestHTTPBackendRef("app-backend-v1", "backend-ns", nil, new(int32(80))),
			...
			referenceGrants: []gwtypes.ReferenceGrant{
				{
					...
					Spec: gwtypes.ReferenceGrantSpec{
						...
						To: []gwtypes.ReferenceGrantTo{
							{Group: "", Kind: "Service", Name: new(gatewayv1.ObjectName("app-backend-v1"))},
						},
					},
				},
			},
...

		{
			...
			httpRoute: createTestHTTPRoute("test-route", "test-namespace", []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("external-service", "", nil, new(int32(80))),
			}),
			backendRefs: []gwtypes.HTTPBackendRef{
				createTestHTTPBackendRef("external-service", "", nil, new(int32(80))),
			},
...
// Helper function to find a target by its address.
func TestTargetsForUDPRouteBackendRefs(t *testing.T) {
	ctx := context.Background()
	logger := logr.Discard()
	port := int32(80)
	backendPort := gwtypes.PortNumber(80)
	...
	endpointSlice := createTestEndpointSlice("test-service-slice", []discoveryv1.EndpointPort{
		createTestEndpointPort("udp", 8080, corev1.ProtocolUDP),
	}, []discoveryv1.Endpoint{
		createTestEndpoint([]string{"10.0.0.2"}, true),
	})
	endpointSlice.Namespace = "test-namespace"
	...
	require.Len(t, targets, 1)
	assert.Equal(t, "10.0.0.2:8080", targets[0].Spec.Target)
	assert.Equal(t, "test-namespace/test-route", targets[0].Annotations["gateway-operator.konghq.com/hybrid-routes-UDPRoute"])
}
...
func TestKongTargetMergeScenarios(t *testing.T) {
	...
	t.Run("single backendRef then overlapping second added: count stays 1, weight is summed", func(t *testing.T) {
		...
		singleRefs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(100)), []string{"10.0.0.1"}),
		}
		...
		overlappingRefs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(60)), []string{"10.0.0.1"}),
			createTestvalidBackendRef("service-b", "test-namespace", new(int32(40)), []string{"10.0.0.1"}),
		}
	...
	t.Run("second backendRef moved to different pod: overlap resolves to 2 targets with distributed weights", func(t *testing.T) {
		// Reconcile 1: two services, same pod IP.
		overlappingRefs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(50)), []string{"10.0.0.1"}),
			createTestvalidBackendRef("service-b", "test-namespace", new(int32(50)), []string{"10.0.0.1"}),
		}
		targets1, err := createTargetsFromValidBackendRefs(ctx, logger, createTestFakeClient(), route, pRef, upstream, overlappingRefs)
		require.NoError(t, err)
		require.Len(t, targets1, 1)
		...
		divergedRefs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(50)), []string{"10.0.0.1"}),
			createTestvalidBackendRef("service-b", "test-namespace", new(int32(50)), []string{"10.0.0.2"}),
		}
...
func TestKongTargetMergeLifecycle(t *testing.T) {
	...
	t.Run("overlap disappears: merged target keeps stable name, new target created for diverged IP", func(t *testing.T) {
		...
		overlapRefs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(50)), []string{"10.0.0.1"}),
			createTestvalidBackendRef("service-b", "test-namespace", new(int32(50)), []string{"10.0.0.1"}),
		}
		...
		splitRefs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(50)), []string{"10.0.0.1"}),
			createTestvalidBackendRef("service-b", "test-namespace", new(int32(50)), []string{"10.0.0.2"}),
		}
	...
	t.Run("overlap introduced on existing in-cluster target: same name returned, weight summed", func(t *testing.T) {
		...
		refs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(30)), []string{"10.0.0.1"}),
			createTestvalidBackendRef("service-b", "test-namespace", new(int32(70)), []string{"10.0.0.1"}),
		}
...
func TestKongTargetNameReuseByAddress(t *testing.T) {
	...
	newExistingTarget := func(name, target string, programmed bool) *configurationv1alpha1.KongTarget {
		...
		tgt.Spec.UpstreamRef = commonv1alpha1.NamespacedRef{Name: upstream}
	...
	t.Run("mints a fresh name when no target exists for the address", func(t *testing.T) {
		...
		refs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(100)), []string{"10.0.0.2"}),
		}
		targets, err := createTargetsFromValidBackendRefs(ctx, logger, cl, route, pRef, upstream, refs)
		require.NoError(t, err)
		require.Len(t, targets, 1)
	...
	t.Run("ignores targets scoped to a different upstream", func(t *testing.T) {
		...
		other.Spec.UpstreamRef = commonv1alpha1.NamespacedRef{Name: "other-upstream"}
		cl := createTestFakeClient(other)
		refs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(100)), []string{"10.0.0.3"}),
		}
		targets, err := createTargetsFromValidBackendRefs(ctx, logger, cl, route, pRef, upstream, refs)
		require.NoError(t, err)
		require.Len(t, targets, 1)
	...
	t.Run("tie-breaks two non-programmed duplicates by smaller name", func(t *testing.T) {
		...
		refs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(100)), []string{"10.0.0.4"}),
		}
		targets, err := createTargetsFromValidBackendRefs(ctx, logger, cl, route, pRef, upstream, refs)
		require.NoError(t, err)
		require.Len(t, targets, 1)
	...
	t.Run("tie-breaks two Programmed duplicates by smaller name", func(t *testing.T) {
		...
		refs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(100)), []string{"10.0.0.5"}),
		}
		targets, err := createTargetsFromValidBackendRefs(ctx, logger, cl, route, pRef, upstream, refs)
		require.NoError(t, err)
		require.Len(t, targets, 1)
	...
	t.Run("list KongTargets error is propagated", func(t *testing.T) {
		...
		refs := []validBackendRef[gwtypes.HTTPBackendRef]{
			createTestvalidBackendRef("service-a", "test-namespace", new(int32(100)), []string{"10.0.0.6"}),
		}
