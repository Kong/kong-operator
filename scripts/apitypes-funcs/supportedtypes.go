...
var supportedConfigurationPackageTypesWithList = func() []supportedTypesT {
	// NOTE: Code below adds types that are generated by the crd-from-oas pipeline
	// and which need a GetItems() method on the list type to satisfy the generic
	// list interface used by tests and watch helpers.
	// When all generation is moved to crd-from-oas this should be removed.
	ret := slices.Clone(supportedKonnectTypesWithControlPlaneConfig)

	idx := slices.IndexFunc(ret, func(st supportedTypesT) bool {
		return st.PackageVersion == "v1alpha1"
	})
	if idx == -1 {
		return ret
	}
	ret[idx].Types = append(ret[idx].Types,
		templateDataT{
...
// supportedKonnectV1Alpha1TypesGeneratedFromOAS lists Konnect v1alpha1 types whose
// type definitions and most helpers are generated by the crd-from-oas pipeline,
// but which still need a GetItems() method on the list type to satisfy the
// generic list interface used by tests and watch helpers.
var supportedKonnectV1Alpha1TypesGeneratedFromOAS = []supportedTypesT{
	{
		PackageVersion: "v1alpha1",
		Types: []templateDataT{
			{
...
var supportedKonnectPackageTypesWithList = func() []supportedTypesT {
	...
	base = append(base, supportedKonnectV1Alpha1TypesGeneratedFromOAS...)
