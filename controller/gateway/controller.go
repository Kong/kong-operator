...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if c, ok := k8sutils.GetCondition(DataPlaneReadyType, gwConditionAware); !ok || c.Status == metav1.ConditionFalse || provisionErr != nil {
		...
		if !ok || c.Reason != string(k8sutils.WaitingToBecomeReadyReason) {
...
func (r *Reconciler) provisionDataPlane(
	...
	if err != nil {
		...
		k8sutils.SetCondition(
			createDataPlaneCondition(metav1.ConditionFalse, k8sutils.UnableToProvisionReason, errWrap.Error(), gateway.Generation),
			gatewayConditionsAndListenersAware(gateway),
		)
		return nil, errWrap
	}
