...
// ensureHPA reconciles the HPA for the given KegDataPlane.
...
func (r *Reconciler) ensureHPA(
	...
	matchingLabels := k8sresources.GetManagedLabelForOwner(egdp)
	hpas, err := k8sutils.ListHPAsForOwner(ctx, r.Client, egdp.Namespace, egdp.UID, matchingLabels)
	if err != nil {
		return fmt.Errorf("failed listing HPAs for KegDataPlane %s/%s: %w", egdp.Namespace, egdp.Name, err)
	}
	...
	if egdp.Spec.Deployment == nil ||
		...
		if err := k8sreduce.ReduceHPAs(ctx, r.Client, hpas, k8sreduce.FilterNone); err != nil {
			return fmt.Errorf("failed reducing HPAs for KegDataPlane %s/%s: %w", egdp.Namespace, egdp.Name, err)
		}
		return nil
	}
	...
	generatedHPA, err := k8sresources.GenerateHPA(egdp, &k8sresources.HPAScalingSpec{
	...
	if err := r.Create(ctx, generatedHPA); err != nil {
		return fmt.Errorf("failed creating HPA for KegDataPlane %s: %w", egdp.Name, err)
	}
	return nil
}
