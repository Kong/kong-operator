...
make verify.crd-breaking-changes  # Check crd-from-oas generated CRDs for breaking schema changes
```
...
If you modify CRDs generated by `crd-from-oas` in `config/crd/kong-operator/`,
...
5. **Run crd-from-oas CRD compatibility checks when needed** - If you modified CRDs generated by `crd-from-oas`:
...
make verify.crd-breaking-changes  # crd-from-oas generated CRDs remain backward compatible
```
...
- Jobs: lint, verify, verify-crd-breaking-changes, unit-tests, envtest-tests, CRDs validation, conformance-tests, integration tests
