...
func createAIGatewayCACertificate(
	ctx context.Context,
	cl client.Client,
	sdk sdkkonnectgo.AIGatewayCACertificatesSDK,
	obj *aiconfigurationv1alpha1.AIGatewayCACertificate,
) error {
	parentID := obj.GetGatewayID()
	if parentID == "" {
		return CantPerformOperationWithoutParentIDError{Entity: obj, Parent: "KonnectAIGateway", Op: CreateOp}
	}
	req, err := obj.ToCreateAIGatewayCACertificateRequest(ctx, cl)
	if err != nil {
	...
	resp, err := sdk.CreateAiGatewayCaCertificate(ctx, parentID, *req)
	if errWrap := wrapErrIfKonnectOpFailed(err, CreateOp, obj); errWrap != nil {
		return errWrap
	}
	if resp == nil || resp.AIGatewayCACertificate == nil || resp.AIGatewayCACertificate.ID == "" {
		return fmt.Errorf("failed creating %s: %w", obj.GetTypeName(), ErrNilResponse)
	}

	obj.SetKonnectID(resp.AIGatewayCACertificate.ID)
	return nil
}

func updateAIGatewayCACertificate(
	ctx context.Context,
	cl client.Client,
	sdk sdkkonnectgo.AIGatewayCACertificatesSDK,
	obj *aiconfigurationv1alpha1.AIGatewayCACertificate,
	...
	req, err := obj.ToUpdateAIGatewayCACertificateRequest(ctx, cl)
	if err != nil {
		return fmt.Errorf("failed building %s SDK update request: %w", obj.GetTypeName(), err)
	}
	req.Labels = WithKubernetesMetadataLabels(obj, req.Labels)

	_, err = sdk.UpdateAiGatewayCaCertificate(ctx, sdkkonnectops.UpdateAiGatewayCaCertificateRequest{
		GatewayID:                           parentID,
		CaCertificateIDOrName:               id,
		UpdateAIGatewayCACertificateRequest: *req,
	})
	if errWrap := wrapErrIfKonnectOpFailed(err, UpdateOp, obj); errWrap != nil {
		return handleUpdateError(ctx, err, obj, func(ctx context.Context) error {
			return createAIGatewayCACertificate(ctx, cl, sdk, obj)
		})
	}
	return nil
}

func deleteAIGatewayCACertificate(
	ctx context.Context,
	sdk sdkkonnectgo.AIGatewayCACertificatesSDK,
	obj *aiconfigurationv1alpha1.AIGatewayCACertificate,
) error {
	...
	_, err := sdk.DeleteAiGatewayCaCertificate(ctx, parentID, id)
...
func getAIGatewayCACertificateForUID(
	ctx context.Context,
	sdk sdkkonnectgo.AIGatewayCACertificatesSDK,
	obj *aiconfigurationv1alpha1.AIGatewayCACertificate,
) (string, error) {
	...
	// TODO: pass a Filter to ListAiGatewayCaCertificates (e.g. by name/labels) so we
	// do not page through every entity in the tenant. Filter types and
	// fields are entity-specific; derive from OpenAPI schema.
	resp, err := sdk.ListAiGatewayCaCertificates(ctx, sdkkonnectops.ListAiGatewayCaCertificatesRequest{
		GatewayID: parentID,
	})
	if err != nil {
	...
	for _, entry := range resp.ListAIGatewayCACertificatesResponse.Data {
