...
// ensureHPA reconciles the HPA for the given MCPServerDataPlane.
// If horizontal scaling is not configured, any existing HPA is deleted.
func (r *MCPServerDataPlaneReconciler) ensureHPA(
	ctx context.Context,
	logger logr.Logger,
	mcpDataPlane *mcpv1alpha1.MCPServerDataPlane,
	deploymentName string,
) error {
	matchingLabels := k8sresources.GetManagedLabelForOwner(mcpDataPlane)
	hpas, err := k8sutils.ListHPAsForOwner(ctx, r.Client, mcpDataPlane.Namespace, mcpDataPlane.UID, matchingLabels)
	if err != nil {
		return fmt.Errorf("failed listing HPAs for MCPServerDataPlane %s/%s: %w", mcpDataPlane.Namespace, mcpDataPlane.Name, err)
	}

	if mcpDataPlane.Spec.Deployment == nil ||
		mcpDataPlane.Spec.Deployment.Scaling == nil ||
		mcpDataPlane.Spec.Deployment.Scaling.HorizontalScaling == nil {
		if err := k8sreduce.ReduceHPAs(ctx, r.Client, hpas, k8sreduce.FilterNone); err != nil {
			return fmt.Errorf("failed reducing HPAs for MCPServerDataPlane %s/%s: %w", mcpDataPlane.Namespace, mcpDataPlane.Name, err)
		}
		return nil
	}
	...
	hs := mcpDataPlane.Spec.Deployment.Scaling.HorizontalScaling
	generatedHPA, err := k8sresources.GenerateHPA(mcpDataPlane, &k8sresources.HPAScalingSpec{
	...
	if err := r.Create(ctx, generatedHPA); err != nil {
		return fmt.Errorf("failed creating HPA for MCPServerDataPlane %s: %w", mcpDataPlane.Name, err)
	}
	return nil
}
