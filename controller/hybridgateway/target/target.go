...
func existingTargetNamesByAddress(
	...
	namespace := metadata.NamespaceFromParentRef(httpRoute, pRef)

	list := &configurationv1alpha1.KongTargetList{}
	if err := cl.List(ctx, list,
		client.InNamespace(namespace),
		metadata.LabelSelectorForOwnedResources(httpRoute, pRef),
	); err != nil {
...
func createTargetsFromValidBackendRefs(ctx context.Context, logger logr.Logger, cl client.Client, httpRoute *gwtypes.HTTPRoute, pRef *gwtypes.ParentReference, upstreamName string,
	...
	existingByAddr, err := existingTargetNamesByAddress(ctx, cl, httpRoute, pRef, upstreamName)
	if err != nil {
		return nil, err
	}
