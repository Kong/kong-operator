...
func (c *httpRouteConverter) translate(ctx context.Context) error {
	...
	if err := c.generateTranslationData(ctx); err != nil {
		return err
	}
	...
	for _, val := range c.ir.Rules {
		...
		for _, match := range val.Matches {
			routeName := match.Name.String()
			serviceName := val.Name.String()
...
func (c *httpRouteConverter) generateTranslationData(ctx context.Context) error {
