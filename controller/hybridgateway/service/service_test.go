...
func TestResolveWriteTimeoutFromHTTPRouteBackendRefs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := resolveWriteTimeoutFromHTTPRouteBackendRefs(ctx, cl, httpRoute, rule, logger)
			if tt.expected == nil {
				assert.Nil(t, result)
			} else {
...
func TestResolveWriteTimeoutFromTLSRouteBackendRefs(t *testing.T) {
	ctx := context.Background()
	logger := zap.New()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := resolveWriteTimeoutFromTLSRouteBackendRefs(ctx, cl, tlsRoute, rule, logger)
			if tt.expected == nil {
				assert.Nil(t, result)
			} else {
				require.NotNil(t, result)
