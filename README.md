...
When you modify CRDs generated by `crd-from-oas`, run `make verify.crd-breaking-changes`
to compare the generated CRDs in `config/crd/kong-operator/` against the base branch before
opening or updating a PR. Override the comparison source with `CRD_BREAKING_CHANGES_BASE_SHA`
or `CRD_BREAKING_CHANGES_BASE_REF` if you need to target a different baseline.
