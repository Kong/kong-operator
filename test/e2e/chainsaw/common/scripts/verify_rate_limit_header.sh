...
#   EXPECTED_LIMIT: The expected rate limit value (e.g., "20" or "30").
...
build_curl_cmd() {
  ...
  CMD="$CMD 'http://${PROXY_HOST}:${PROXY_PORT}${ROUTE_PATH}'"
  echo "$CMD"
}
...
for ATTEMPT in $(seq 1 $MAX_RETRIES); do
  ...
  # Extract the rate limit header, removing carriage returns.
  HEADER=$(echo "$RESPONSE" | grep -i "X-RateLimit-Limit-Minute:" | tr -d '\r' || echo "")
  LAST_HEADER="$HEADER"

  # Check if header matches expected value.
  if [[ "$HEADER" == "X-RateLimit-Limit-Minute: $EXPECTED_LIMIT" ]]; then
    cat <<EOF
{
  "success": true,
  "expected_limit": "$EXPECTED_LIMIT",
  ...
  "message": "Rate limit header matched on attempt $ATTEMPT",
...
cat <<EOF
{
  ...
  "max_retries": $MAX_RETRIES,
  "error": "Header did not match expected value after $MAX_RETRIES attempts",
  "proxy_ip": "$PROXY_IP",
