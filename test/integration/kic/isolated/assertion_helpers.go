...
func assertEventuallyNoResponseUDP(t *testing.T, udpGatewayURL string) {
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		err := helpers.EchoResponds(t, helpers.ProtocolUDP, udpGatewayURL, "irrelevant")
...
func assertEventuallyResponseUDP(t *testing.T, udpGatewayURL, expectedMsg string) {
	t.Helper()
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assert.NoError(c, helpers.EchoResponds(t, helpers.ProtocolUDP, udpGatewayURL, expectedMsg))
	}, consts.IngressWait, consts.WaitTick)
}
...
func assertEventuallyNoResponseTCP(t *testing.T, tcpGatewayURL string) {
	t.Helper()
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		err := helpers.EchoResponds(t, helpers.ProtocolTCP, tcpGatewayURL, "irrelevant")
...
func assertEventuallyResponseTCP(t *testing.T, tcpGatewayURL, expectedMsg string) {
	t.Helper()
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assert.NoError(c, helpers.EchoResponds(t, helpers.ProtocolTCP, tcpGatewayURL, expectedMsg))
	}, consts.IngressWait, consts.WaitTick)
}
